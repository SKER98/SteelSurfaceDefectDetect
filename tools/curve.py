...
CONFIG_FILE = f"../configs/neu_det/swint_3x_neudet.py"
RESULT_FILE = f"../output/swin.pkl"
...
def plot_recall(config_file, result_file, metric="bbox"):
	result_file = ["../output/swin.pkl", "../output/fasterrcnn.pkl", "../output/reppoint.pkl", "yolov3.pkl"]
	...
	# load result file in pkl format
	pkl_results = mmcv.load(result_file[0])
	# convert pkl file (list[list | tuple | ndarray]) to json
	json_results, _ = dataset.format_results(pkl_results)
	coco_dt = coco_gt.loadRes(json_results[metric])
	# initialize COCOeval instance
	...
	eval_recalls()
	plot_iou_recall(recall,[0.5,0.6,0.7])


if __name__ == "__main__":
	plot_recall(config_file=CONFIG_FILE, result_file=RESULT_FILE, metric="bbox")
